...
최종 피드백 반영:
1. 원본 템플릿의 폰트 weight 정확히 매칭:
   - 제품명: font-weight: 600 (Semibold)
   - 섹션 헤더: font-weight: 400 (Regular)
   - 일반 텍스트: weight 지정 없음 (body default)
   - font-weight: 300은 거의 사용되지 않음
2. 템플릿의 폰트 색상 정확히 매칭:
   - #222 (진한 회색): 제품명, 컬러명, 디테일 설명
   - #666 (중간 회색): 모델 스펙, Care 내용, Check Point 내용
   - black (default): Color Selector 항목 등
3. Gallery 헤더에 모델 스펙 추가 (우상단)
4. 섹션 간격 추가 (각 섹션 끝에 240px)

실행 방법:
  python examples/generate_figma_continuous.py
...
def generate_html(product, sheets_loader):
    ...
    # 3. 갤러리 이미지 (Lifestyle Gallery 섹션) - 컬러칩 + 모델 스펙 정보 포함
    ...
    # 모델 정보 추출 (Gallery 헤더에 표시)
    model_spec = ""
    if product.model_info and len(product.model_info) > 0:
        first_model = product.model_info[0]
        model_spec = f"Model : {first_model.model_measurements} / {first_model.model_size}"
...
"""
    ...
    if main_image_base64:
        html += f"""
            <div class="section section--product-hero" style="position: relative; width: 1033px; height: 1849px; margin: 0 auto; margin-bottom: 240px;">
...
"""
    ...
    if color_images_base64:
        html += """
            <div class="section section--color-variants" style="position: relative; width: 1082px; margin: 20px auto 0; margin-bottom: 240px;">
...
"""
    ...
    # Section 3: Lifestyle Gallery (갤러리) - 컬러칩 + 모델 스펙 추가
    if gallery_images_base64:
        html += """
            <div class="section section--lifestyle-gallery" style="position: relative; width: 1042px; margin: 20px auto 0; margin-bottom: 240px;">
"""
        for color_name, color_data in gallery_images_base64.items():
            # 컬러 헤더 (컬러칩 + 컬러명 + 모델 스펙)
            html += f"""
                <div style="display: flex; justify-content: space-between; align-items: center; height: 67px; padding: 0 24px; margin-bottom: 20px;">
                    ...
                    <div style="font-size: 28px; color: #666;">{model_spec}</div>
                </div>
...
"""
    ...
    if detail_images_base64:
        html += """
            <div class="section section--material-detail" style="position: relative; width: 1042px; margin: 20px auto 0; margin-bottom: 240px;">
...
"""
    ...
    if product.colors:
        html += """
            <div class="section section--color-selector" style="position: relative; width: 1044px; margin: 20px auto 0; margin-bottom: 240px;">
...
"""
    ...
    html += f"""
            <div class="section section--product-info" style="position: relative; width: 1044px; margin: 20px auto 0; margin-bottom: 240px;">
...
"""
    ...
    if product.checkpoint and checkpoint_image_base64:
        html += f"""
            <div class="section section--care-instructions" style="position: relative; width: 1044px; margin: 20px auto 0; margin-bottom: 240px;">
...
"""
    ...
    if product.model_info:
        html += """
            <div class="section section--model-info" style="position: relative; width: 1044px; margin: 20px auto 0; margin-bottom: 240px;">
...
def main():
    ...
    print("=" * 60)
    print("🎨 Figma Continuous 템플릿 기반 HTML 페이지 생성 (최종)")
    print("=" * 60)
    ...
    try:
        ...
        print()
        print("✅ 적용된 수정사항:")
        print("  1. 원본 템플릿의 폰트 weight 정확히 매칭")
        print("     - 제품명: font-weight: 600 (Semibold)")
        print("     - 섹션 헤더: font-weight: 400 (Regular)")
        print("     - 일반 텍스트: weight 지정 없음")
        print("  2. 원본 템플릿의 폰트 색상 정확히 매칭")
        print("     - #222: 제품명, 컬러명, 디테일 설명")
        print("     - #666: 모델 스펙, Care/Check Point 내용")
        print("  3. Gallery 헤더에 모델 스펙 추가 (우상단)")
        print("  4. 섹션 간격 추가 (각 섹션 끝 240px)")
        print()
        print("브라우저에서 확인:")
