"""
Figma 템플릿 기반 HTML 페이지 생성

실행 방법:
  python examples/generate_figma_html.py
...
def generate_html(product, sheets_loader):
    """ProductData와 figma 템플릿을 사용하여 HTML 생성"""
    ...
    # 3. 갤러리 이미지 (Lifestyle Gallery 섹션)
    ...
    for color_name, images in product.gallery_by_color.items():
        ...
        if base64_list:
            gallery_images_base64[color_name] = base64_list
...
<body>
    <div class="info">
        ...
        <p><strong>갤러리:</strong> {sum(len(imgs) for imgs in product.gallery_by_color.values())}장</p>
...
"""
    ...
    if color_images_base64:
        html += """
            <div class="section section--color-variants" style="position: relative; width: 1082px; margin: 20px auto 0; padding: 20px; background: #FFF3E5;">
        ...
        for color_data in color_images_base64:
            html += f"""
                    <div style="display: flex; flex-direction: column; gap: 15px;">
                        ...
                        <div style="text-align: center; font-size: 36px; color: #222;">{color_data['name']}</div>
                    </div>
"""
        html += """
                </div>
...
"""
    ...
    # Section 3: Lifestyle Gallery (갤러리)
    if gallery_images_base64:
        html += """
            <div class="section section--lifestyle-gallery" style="position: relative; width: 1042px; margin: 20px auto 0; padding: 20px; background: #E5FFE5;">
"""
        for color_name, images in gallery_images_base64.items():
            # 컬러 헤더
            html += f"""
...
"""
    ...
    if detail_images_base64:
        html += """
            <div class="section section--material-detail" style="position: relative; width: 1042px; margin: 20px auto 0; padding: 20px; background: #F3E5FF;">
        ...
        for detail in detail_images_base64:
            html += f"""
                    <div style="display: flex; flex-direction: column; gap: 29px;">
                        ...
                        <div style="text-align: center; font-size: 36px; color: #222;">{detail['text']}</div>
                    </div>
"""
        html += """
                </div>
...
"""
    ...
    if product.colors:
        html += """
            <div class="section section--color-selector" style="position: relative; width: 1044px; margin: 20px auto 0; padding: 20px; background: #FFE5F3;">
        ...
        for color in product.colors:
            html += f"""
                    <div style="display: flex; gap: 10px; align-items: center;">
                        ...
                        <span style="font-size: 24px;">{color.color_name}</span>
                    </div>
"""
        html += """
                </div>
...
"""
    ...
    html += f"""
            <div class="section section--product-info" style="position: relative; width: 1044px; margin: 20px auto 0; padding: 20px; background: #FFFFE5;">
                <div style="font-size: 40px; font-weight: 400; margin-bottom: 46px;">Fabric</div>
                <div style="font-size: 26px; color: #222; margin-bottom: 20px;">{product.fabric_info.fabric_composition or 'N/A'}</div>
                ...
                <div style="display: flex; gap: 120px; align-items: flex-start; margin-top: 46px;">
                    ...
                    <div style="font-size: 26px; color: #666; line-height: 1.6; flex: 1;">
...
"""
    ...
    if product.checkpoint and checkpoint_image_base64:
        html += f"""
            <div class="section section--care-instructions" style="position: relative; width: 1044px; margin: 20px auto 0; padding: 20px; background: #E5FFFF;">
                <div style="display: flex; gap: 120px; align-items: flex-start;">
                    <div style="font-size: 36px; font-weight: 400; width: 100px;">Check<br/>Point</div>
...
"""
    ...
    if product.model_info:
        html += """
            <div class="section section--model-info" style="position: relative; width: 1044px; margin: 20px auto 0; padding: 20px; background: #F3FFE5;">
        ...
        for model in product.model_info:
            html += f"""
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                        ...
                        <div style="text-align: center; font-size: 26px; color: #666;">{model.model_measurements} / {model.model_size}</div>
                    </div>
"""
        html += """
                </div>
...
"""
    ...
    if product.size_info.top or product.size_info.bottom:
        html += """
            <div class="section section--size-chart" style="position: relative; width: 1044px; margin: 20px auto 0; padding: 20px; background: #E5E5FF;">
                <div style="font-size: 36px; font-weight: 400; margin-bottom: 20px;">Size Information <span style="font-size: 26px; color: #666;">(cm)</span></div>
"""
            ...
            html += """
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                    <thead style="background: #f0f0f0;">
                        <tr>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">사이즈</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">어깨</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">가슴</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">소매</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">총장</th>
                        </tr>
                    </thead>
                    <tbody>
"""
            for size in product.size_info.top:
                html += f"""
                        <tr>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.size_name}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.shoulder}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.chest}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.sleeve}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.length}</td>
                        </tr>
"""
            html += """
                    </tbody>
            ...
            html += """
                <table style="width: 100%; border-collapse: collapse;">
                    <thead style="background: #f0f0f0;">
                        <tr>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">사이즈</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">허리</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">힙</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">허벅지</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">밑단</th>
                            <th style="padding: 10px; border: 1px solid #ddd; font-size: 24px;">밑위</th>
                        </tr>
                    </thead>
                    <tbody>
"""
            for size in product.size_info.bottom:
                html += f"""
                        <tr>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.size_name}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.waist}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.hip}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.thigh}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.hem}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; text-align: center; font-size: 22px;">{size.rise}</td>
                        </tr>
"""
            html += """
                    </tbody>
...
def main():
    ...
    print("=" * 60)
    print("🎨 Figma 템플릿 기반 HTML 페이지 생성")
    print("=" * 60)
